...
const usageLogSelectColumns = "id, user_id, api_key_id, account_id, request_id, model, requested_model, upstream_model, group_id, subscription_id, input_tokens, output_tokens, cache_creation_tokens, cache_read_tokens, cache_creation_5m_tokens, cache_creation_1h_tokens, image_output_tokens, image_output_cost, input_cost, output_cost, cache_creation_cost, cache_read_cost, total_cost, actual_cost, rate_multiplier, account_rate_multiplier, billing_type, request_type, stream, openai_ws_mode, duration_ms, first_token_ms, user_agent, ip_address, image_count, image_size, service_tier, reasoning_effort, inbound_endpoint, upstream_endpoint, cache_ttl_overridden, channel_id, model_mapping_chain, billing_tier, billing_mode, conn_pick_ms, queue_wait_ms, account_recheck_ms, created_at"
...
var usageLogInsertArgTypes = [...]string{
	...
	"text",        // billing_tier
	"text",        // billing_mode
	"integer",     // conn_pick_ms
	"integer",     // queue_wait_ms
	"integer",     // account_recheck_ms
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			billingTier,
			billingMode,
			nullInt(log.ConnPickMs),
			nullInt(log.QueueWaitMs),
			nullInt(log.AccountRecheckMs),
			createdAt,
		},
	}
}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		connPickMs            sql.NullInt64
		queueWaitMs           sql.NullInt64
		accountRecheckMs      sql.NullInt64
		createdAt             time.Time
	)
	...
	if connPickMs.Valid {
		value := int(connPickMs.Int64)
		log.ConnPickMs = &value
	}
	if queueWaitMs.Valid {
		value := int(queueWaitMs.Int64)
		log.QueueWaitMs = &value
	}
	if accountRecheckMs.Valid {
		value := int(accountRecheckMs.Int64)
		log.AccountRecheckMs = &value
	}
